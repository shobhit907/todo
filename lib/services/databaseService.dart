...
class DatabaseService {
  static String _dbName = 'temp4.db';
  ...
  Future<Database> _initDb() async {
    ...
    return await openDatabase(path,
        version: 1, onCreate: (Database _db, int version) async{
      await _db.execute(
  ...
  Future<Task> getTask(int idx)async{
    _db=await db;
    List<Map<String,dynamic>> _queryResult=await _db.query(_tasksTable,where: "id=$idx");
  ...
  Future<Task> getSubTask(int idx)async{
    _db=await db;
    List<Map<String,dynamic>> _queryResult=await _db.query(_subtasksTable,where: "id=$idx");
    if(_queryResult.length>0){
      return Task.fromMap(_queryResult[0]);
    }else{
      return null;
  ...
  Future<List<Task>> getAllSubTasks(List<int> idx)async{
    _db=await db;
    List<Map<String,dynamic>> _allSubTasks=await _db.query(_subtasksTable,where: "id IN (${idx.join(', ')})");
    List<Task> _ret=new List();
